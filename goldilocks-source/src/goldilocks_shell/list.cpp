...
{  
    std::cout<<"get_suite called with container size: "<< suites->suites_send_suites().size()<<'\n';
    if(!suites->suites_send_suites().empty()){
        for(auto& suite: suites->suites_send_suites()){
