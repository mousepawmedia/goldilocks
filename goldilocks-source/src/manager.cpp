...
void TestManager::i_load_suite(testsuitename_t suite)
{
    ...
    {
        channel << IOCat::error << IOFormatTextAttr::bold << IOFormatTextFG::red << "ERROR: The suite " << suite
    ...
    channel << IOFormatTextAttr::bold << suites[suite]->get_title() << IOCtrl::end;
...
void TestManager::i_run_test(testname_t test, unsigned int repeat)
{
    ...
    channel << IOFormatTextAttr::bold << tests[test]->get_title() << IOCtrl::end;
    channel << "[" << test << "]" << IOCtrl::end;
    if(repeat > 1)
    {
        channel << " " << stringify(repeat, IOFormatBase::dec) << " times" << IOCtrl::end;
    }
...
void TestManager::i_run_benchmark(testname_t test, unsigned int repeat)
{
    ...
    channel << IOFormatTextAttr::bold << comparatives[test]->get_title() << IOCtrl::end;
...
void TestManager::i_run_compare(testname_t test1, testname_t test2, unsigned int repeat, bool showResults)
{
    ...
    channel << "Run comparative benchmark between " << IOCtrl::end;
    channel << IOFormatTextAttr::bold << tests[test1]->get_title() << IOCtrl::end;
    channel << "[" << test1 << "] and " << IOCtrl::end;
    channel << IOFormatTextAttr::bold << tests[test2]->get_title() << IOCtrl::end;
    channel << "[" << test2 << "] at " << repeat << " repetitions? (y/N) " << IOCtrl::end;
...
bool TestManager::load_suite(testsuitename_t suite)
{
    ...
    if(suite == "")
    {
        ...
        for(std::map<testsuitename_t, testsuiteptr_t>::iterator it = suites.begin(); it != suites.end(); ++it)
        {
            ...
            if(it->second->is_loaded() == false)
            {
                ...
                channel << IOCat::normal << IOFormatTextAttr::bold << it->first << ": " << it->second->get_title() << " loaded." << IOCtrl::endl;
            }
        }
        return true;
    }
    ...
    if(!suites[suite]->is_loaded())
    {
        ...
        channel << IOCat::normal << IOFormatTextAttr::bold << suites[suite]->get_title() << IOCtrl::endl;
        channel << IOCat::normal << IOFormatTextAttr::bold << IOFormatTextBG::green << IOFormatTextFG::white
...
bool TestManager::run_suite(testsuitename_t suite)
{
    ...
    channel << "===== [" << suites[suite]->get_title() << "] =====" << IOCtrl::endl;
    ...
    if(!r)
    {
        channel << IOCtrl::n << IOCtrl::clear << IOCat::error << IOFormatTextAttr::bold << IOFormatTextBG::red << IOFormatTextFG::white
    ...
    else
    {
        ...
        channel << IOCtrl::n << IOCtrl::clear << IOCat::normal << IOFormatTextAttr::bold << IOFormatTextBG::green << IOFormatTextFG::white
...
bool TestManager::run_test(testname_t test, unsigned int repeat)
{
    ...
    {
        // Display an error.
        channel << IOCat::error << "Test repeat value must be between 1 and 10000 inclusively." << IOCtrl::endl;
    ...
    channel << "===== [" << tests[test]->get_title() << "] =====" << IOCtrl::endl;
    ...
    if(!(tests[test]->pre()))
    {
        ...
        channel << IOCat::error << IOFormatTextAttr::bold << IOFormatTextBG::red << IOFormatTextFG::white
    ...
    {
        channel << IOCat::normal << "Pass " << stringify(i+1, IOFormatBase::dec)
            << " of " << stringify(repeat, IOFormatBase::dec) << IOCtrl::endl;
...
void TestManager::benchmark_banner()
{
    ...
    channel << IOFormatTextAttr::bold << IOFormatTextFG::blue
        ...
        << "|     " << "BENCHMARKER" << "    |\n"
        << "======" << "===========" << "=====" << IOCtrl::endl;
    channel << IOFormatTextAttr::none << IOFormatTextFG::red
    ...
    bool showStats, bool deadHeat)
{
    ...
    channel << "Ensuring [" << test1->get_title() << "] succeeds before benchmarking..." << IOCtrl::endl;
    ...
    channel << "Ensuring [" << test2->get_title() << "] succeeds before benchmarking..." << IOCtrl::endl;
    ...
    channel << IOCat::normal << IOFormatTextBG::cyan << IOFormatTextFG::white << "STARTING BENCHMARK" << IOCtrl::endl;
    ...
    for(unsigned int i=0; i<repeat; ++i)
    {
        ...
        channel << IOCat::normal << "CALIBRATING: " << (((i+1)/repeat)*100) << "%" << IOCtrl::endc;
    ...
    for(unsigned int i=0; i<repeat; ++i)
    {
        ...
        channel << IOCat::normal << "(MAMA BEAR) Pass " << (i+1) << "-A of " << repeat << ".  " << IOCtrl::endc;
        ...
        // Display progress (overwrite line.)
        channel << IOCat::normal << "(MAMA BEAR) Pass " << (i+1) << "-B of " << repeat << ".  " << IOCtrl::endc;
    ...
    * in other words, allowing natural cache warming to occur.*/
    ...
    {
        // Display progress (overwrite line).
        channel << IOCat::normal << "(PAPA BEAR) Pass " << (i+1) << "-A of " << repeat << ".  " << IOCtrl::endc;
        // Run pre-repeat cleanup on test1.
        test1->janitor();
        // Record test 1 measurement, offset by baseline.
        results1[i] = clock(test1) - base;
    ...
    {
        // Display progress (overwrite line).
        channel << IOCat::normal << "(PAPA BEAR) Pass " << (i+1) << "-B of " << repeat << ".  " << IOCtrl::endc;
        // Run pre-repeat cleanup on test2.
        test2->janitor();
        // Record test 2 measurement, offset by baseline.
        results2[i] = clock(test2) - base;
    ...
    for(int o=0; o<outer; o++)
    {
        ...
        {
            // Display progress (overwrite line.)
            channel << IOCat::normal << "(BABY BEAR) Pass " << ((o+1)*(i1+1)) << "-A of " << repeat << ".  " << IOCtrl::endc;
            // Run pre-repeat cleanup on test1.
            test1->janitor();
            // Record test 1 measurement, offset by baseline.
        ...
        {
            // Display progress (overwrite line.)
            channel << IOCat::normal << "(BABY BEAR) Pass " << ((o+1)*(i2+1)) << "-B of " << repeat << ".  " << IOCtrl::endc;
            // Run pre-repeat cleanup on test2.
            test2->janitor();
            // Record test 2 measurement, offset by baseline.
    ...
    {
        // Display progress (overwrite line.)
        channel << IOCat::normal << "(BABY BEAR) Pass " << ((outer*inner)+(r1+1)) << "-A of " << repeat << ".  " << IOCtrl::endc;
        // Run pre-repeat cleanup on test1.
        test1->janitor();
        // Record test 1 measurement, offset by baseline.
    ...
    {
        // Display progress (overwrite line.)
        channel << IOCat::normal << "(BABY BEAR) Pass " << ((outer*inner)+(r2+1)) << "-B of " << repeat << "." << IOCtrl::sendc;
        // Run pre-repeat cleanup on test2.
        test2->janitor();
        // Record test 2 measurement, offset by baseline.
...
void TestManager::printResult(BenchmarkResult& result)
{
    ...
    channel << "\tMEAN (μ): " << result.mean << " / " << result.mean_adj << IOCtrl::sendl;

    channel <<  "\tMIN-MAX(RANGE): " << result.min_val << "-" << result.max_val << "(" << result.range << ") / "
        <<  result.min_adj_val << "-" << result.max_adj_val << "(" << result.range_adj <<")" << IOCtrl::sendl;

    channel << "\tOUTLIERS: " << result.low_out_major+result.low_out_minor << " LOW, "
        << result.upp_out_major+result.upp_out_minor << " HIGH" << IOCtrl::sendl;

    channel << "\tSD (σ): " << IOFormatDecimalPlaces(2) << result.std_dev << " / " << result.std_dev_adj << "" << IOCtrl::sendl;

    channel << "\t" << (result.rsd_adj > rsd_threshold ? IOFormatTextBG::red : IOFormatTextBG::none)
...
void TestManager::printVerdict(BenchmarkResult& result1, BenchmarkResult& result2, Test* test1, Test* test2)
{
    ...
    if(labs(difference) <= result1.std_dev || labs(difference) <= result2.std_dev)
    {
        ...
        channel << "\t     RAW: Both tests are roughly identical. (DIFF <= STD DEV)" << IOCtrl::sendl;
    }
    else
    {
        ...
        if(difference < 0)
        {
            ...
            channel << "\t     RAW: [" << test1->get_title() << "] faster by approx. " << (labs(difference)) << " cycles." << IOCtrl::sendl;
        }
        //Else if the second test won (its non-adjusted mean was smaller)...
        else if(difference > 0)
        {
            // Declare the second test as faster, and by how much.
            channel << "\t     RAW: [" << test2->get_title() << "] faster by approx. " << (labs(difference)) << " cycles." << IOCtrl::sendl;
        }
    }
    ...
    if(labs(difference_adj) <= result1.std_dev_adj || labs(difference_adj) <= result2.std_dev_adj)
    {
        ...
        channel << "\tADJUSTED: Both tests are roughly identical. (DIFF <= STD DEV)" << IOCtrl::endl;
    }
    else
    {
        //If the first test won (its adjusted mean was smaller)...
        ...
        {
            // Declare the first test as faster, and by how much.
            channel << "\tADJUSTED: [" << test1->get_title() << "] faster by approx. " << (labs(difference_adj) - result1.std_dev_adj) << " cycles." << IOCtrl::endl;
        }
        //Else if the second test won (its adjusted mean was smaller)...
        else if(difference_adj > 0)
        {
            // Declare the second test as faster, and by how much.
            channel << "\tADJUSTED: [" << test2->get_title() << "] faster by approx. " << (labs(difference_adj) - result2.std_dev_adj) << " cycles." << IOCtrl::endl;
        }
    }
}
...
bool TestManager::validate(testname_t item_name, bool yell, GolidlocksItemType type)
{
    ...
    if(yell && !r)
    {
        switch(type)
        {
            case GolidlocksItemType::test:
            case GolidlocksItemType::suite:
                channel << IOCat::error << IOFormatTextAttr::bold << IOFormatTextFG::red << "ERROR: The item " << item_name
