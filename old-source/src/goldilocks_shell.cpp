...
int GoldilocksShell::command(int argc_s, char* argv[], unsigned int skip)
{
	...
	for (unsigned int i = 0; i < args; ++i) {
		...
		if (tokens[i] == "--help") {
			// Display CLI help
			out << "Usage: ./goldilocks-tester [command] [argument]"
				<< IOCtrl::n;
			...
			out << "    --run [item]    | Run the test or suite [item]. "
				<< IOCtrl::n;
			out << "    --benchmark [item]    | Runs the benchmark for [item]. "
				<< IOCtrl::n;
			out << IOCtrl::endl;
		}
		...
		// If benchmark flag and subsequent argument are provided...
			 ...
			 */
			r = this->testmanager->run_benchmark(tokens[++i], 100, true, true)
					? 0
					: 1;
		}
...
void GoldilocksShell::interactive()
{
	...
	while (tokens[0] != "exit") {
		if (tokens[0] == "help") {
			...
			out << IOFormatTextFG::yellow << IOFormatTextAttr::bold
				<< IOCat::warning << "[Deprecated] Use 'run [suite]' instead."
				<< IOCtrl::endl;
			run(tokens);
...
void GoldilocksShell::help()
{
	...
	out << IOFormatTextAttr::bold << this->prompt << "about [test]"
		<< IOCtrl::endl;
	out << "Display the documentation for [test].\n" << IOCtrl::endl;
	out << "----" << IOCtrl::endl;

	out << IOFormatTextAttr::bold << this->prompt << "benchmark [test] (number)"
		<< IOCtrl::endl;
	out << "Runs a benchmark on [test] with (number) repetitions.\n"
		<< IOCtrl::endl;
	out << "----" << IOCtrl::endl;

	out << IOFormatTextAttr::bold << this->prompt
		<< "compare [test1] [test2] (number)" << IOCtrl::endl;
	out << "Runs a comparative benchmark between [test1] and [test2] with "
		   "(number) reps.\n"
		<< IOCtrl::endl;
	out << "----" << IOCtrl::endl;
	...
	out << IOFormatTextAttr::bold << this->prompt << "listsuites"
		<< IOCtrl::endl;
	...
	out << IOFormatTextAttr::bold << this->prompt << "load [suite]"
		<< IOCtrl::endl;
	out << "Loads the tests from [suite].\n" << IOCtrl::endl;
	out << "----" << IOCtrl::endl;

	out << IOFormatTextAttr::bold << this->prompt << "run [suite]"
		<< IOCtrl::endl;
	out << "Runs all tests in [suite].\n" << IOCtrl::endl;
	out << "----" << IOCtrl::endl;

	out << IOFormatTextAttr::bold << this->prompt << "run [test] (number)"
		<< IOCtrl::endl;
...
void GoldilocksShell::compare(stringvector& tokens)
{
	...
	if (args == 2) {
		...
		try {
			this->testmanager->i_run_compare(tokens[1],
											 tokens[2],
											 std::stoi(tokens[3]));
		} catch (const std::invalid_argument&) {
			out << IOFormatTextFG::red << IOFormatTextAttr::bold << IOCat::error
